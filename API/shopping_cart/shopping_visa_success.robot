...
Search
    ...
    ${set}    Set Variable    ${resp.json()['products'][1]}
    Should Be Equal    ${set['product_name']}    43 Piece dinner Set
    Should Be Equal As Numbers   ${set['product_price']}    12.95
    Should Be Equal    ${set['product_image']}    /43_Piece_dinner_Set.png

Product Detail
    ${resp}=    Get Request    alias=shopping    uri=/api/v1/product/2    headers=&{headers}
    Request Should Be Successful    ${resp}
    ${set}    Set Variable    ${resp.json()}
    Should Be Equal    ${set['product_name']}    43 Piece dinner Set
    Should Be Equal As Numbers   ${set['product_price']}    12.95
    Should Be Equal    ${set['product_image']}    /43_Piece_dinner_Set.png
    Should Be Equal As Integers   ${set['quantity']}    10
    Should Be Equal    ${set['product_brand']}    CoolKidz
...
Submit Order
    ...
    Request Should Be Successful    ${resp}
    ${set}    Set Variable    ${resp.json()}
    Should Be Equal As Integers    ${set['order_id']}    8004359122
    Should Be Equal As Numbers    ${set['total_price']}    14.95
...
Confirm Payment
    ...
    Request Should Be Successful    ${resp}
    ${set}    Set Variable    ${resp.json()}
    Should Be Equal    ${set['notify_message']}    วันเวลาที่ชำระเงิน 1/3/2020 13:30:00 หมายเลขคำสั่งซื้อ 8004359122 คุณสามารถติดตามสินค้าผ่านช่องทาง Kerry หมายเลข 1785261900